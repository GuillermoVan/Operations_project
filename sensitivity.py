...
#         sensitivity_range = [0.75, 1.0, 1.25, 1.5]          # range factors for C
#         # sensitivity_range = [0.8, 1.0, 1.2]

#         # Choose parameters for sensitivity
#         # parameters_to_test = ['s_open', 's_operate', 'h0']  # COST PARAMETERS
#         parameters_to_test = ['h0', 's_operate', 's_open']  # COST PARAMETERS
...
#         bars1 = ax.bar(positions, waiting_cost, width=bar_width, label='Waiting Cost')
#         bars2 = ax.bar(positions, opening_cost, width=bar_width, bottom=waiting_cost, label='Opening Cost')
#         bars3 = ax.bar(positions, operating_cost, width=bar_width, bottom=[i + j for i, j in zip(waiting_cost, opening_cost)], label='Operating Cost')
...
#                     f'{wait_pct:.1f}%', ha='center', va='bottom', color='white')
#             ax.text(bar2.get_x() + bar2.get_width() / 2, height1 + height2 / 2,
#                     f'{open_pct:.1f}%', ha='center', va='bottom', color='white')
#             ax.text(bar3.get_x() + bar3.get_width() / 2, height1 + height2 + height3 / 2,
#                     f'{op_pct:.1f}%', ha='center', va='bottom', color='white')
...
class Sensitivity:
    ...
    def sensitivity_analysis(self):
        # Define the range for sensitivity analysis
        # sensitivity_range = [0.5, 0.75, 1.0, 1.25, 1.5]  # range for factors
        sensitivity_range = [1.0001, 1.0005, 1.001, 1.0015, 1.002]

        # feasible range 'p' : 0.9980 - 1.0000...
        # feasible range 'l' : ? - 1.002 (or larger...)

        # Choose parameters for sensitivity
        # parameters_to_test = ['s_open', 's_operate', 'h0']  # COST PARAMETERS
        parameters_to_test = ['l']
